...
Status OnLanePlanning::Init(const PlanningConfig& config) {
  ...
  //TaskFactory任务工厂在PlannningBase::Init进行初始化
  PlanningBase::Init(config_);

  //planner_dispatcher在OnLanePlanning构造函数中创建
  //planner_factory_路径规划工厂类注册，包括rtk,public_road,lattice等
  planner_dispatcher_->Init();
  //交通规划配置文件 modules/planning/conf/traffic_rule_config.pb.txt
  ...
  // dispatch planner
  // 创建planner_ogbject,在这里配置文件是PUBLIC_ROAD planner
...
//OnLanePlanning的主要逻辑为事件触发，即收到相应的消息后就触发执行
//这么做的好处是增加了实时性
void OnLanePlanning::RunOnce(const LocalView& local_view,
  ...
  const double start_timestamp = Clock::NowInSeconds(); //路径规划起始时间
  //路径规划开始时间，计算路径规划算法耗时
  ...
  if (!status.ok() || !util::IsVehicleStateValid(vehicle_state)) {
    ...
    //生成刹车轨迹，重点参看protobuf是如何赋值的，具有代表性
    GenerateStopTrajectory(ptr_trajectory_pb);
    return;
  }

  //如果车辆状态时间戳与开始时间戳差20ms,更新预测车辆状态，吧车辆状态预测到当前时间
  ...
  //判断上一routing和当前routing是否相同，相同返回false,不同返回true
  ...
  //重新规划路径的原因，拼接轨迹，使得上一帧和当前帧的轨迹是连续的
  ...
  //设置车辆起始点，设置vehicle_state，计算本车box
  injector_->ego_info()->Update(stitching_trajectory.back(), vehicle_state);
  const uint32_t frame_num = static_cast<uint32_t>(seq_num_++);
  //stitching_trajectory.back()为本周期局部路径规划的起点
  status = InitFrame(frame_num, stitching_trajectory.back(), vehicle_state);

  //根据障碍物计算车前最近障碍物的距离，OK可
  if (status.ok()) {

...
bool HybridAStar::Plan(
  ...
  for (const auto& obstacle_vertices : obstacles_vertices_vec) {
    ...
    std::vector<common::math::LineSegment2d> obstacle_linesegments; //构造障碍物轮廓线段
  ...
  if (!ValidityCheck(start_node_)) {  // 检查node有效性
  ...
  // 使用动态规划DP来计算目标点到某点的启发代价（以目标点为DP的起点），即A*中的H生成graph的同时获得了目标点到图中任一点
  // 的cost，作为缓存，这就是DPMap的作用

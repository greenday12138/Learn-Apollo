...
bool DiscretePointsReferenceLineSmoother::Smooth(
  ...
  // road
  anchorpoints_lateralbound.front() = 0.0;  //起始点横向约束为0
  anchorpoints_lateralbound.back() = 0.0; //终点横向约束为0

  //针对第一个点做差，所有点的坐标都减去第一个点的坐标
...
bool DiscretePointsReferenceLineSmoother::FemPosSmooth(
  ...
  FemPosDeviationSmoother smoother(fem_pos_config); //构造函数，添加配置文件

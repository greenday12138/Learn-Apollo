...
void ReferenceLineProvider::GenerateThread() {
  ...
  //stop函数在OnLinePlanning::~OnLanePlanning中调用
  while (!is_stop_) {
    ...
    if (!CreateReferenceLine(&reference_lines, &segments)) {
      //创建参考线失败 is_reference_line_update_为atomic参数
      is_reference_line_updated_ = false;
      AERROR << "Fail to get reference line";
      continue;
    }
    //更新参考线，主要是更新reference_lines_,route_segments_
...
bool ReferenceLineProvider::CreateReferenceLine(
  ...
  bool is_new_routing = false;
  {
    ...
    std::lock_guard<std::mutex> lock(pnc_map_mutex_);
    //检查routing是否与PncMap中现有的Routing相同
    if (pnc_map_->IsNewRouting(routing)) {
      //如果是新的routing,则更新routing信息
      is_new_routing = true;
